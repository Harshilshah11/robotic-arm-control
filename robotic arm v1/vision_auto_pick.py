...
class Joint:
    def __init__(self, dir, pulse, minDegree, maxDegree, maxPulse, degreeToPulseRatio):
...
def pick_place():
    ...
    if solve_d3(saved_movement_1[1]) is not None:
        move_stepper(saved_movement_1[1], joint2)

    if solve_d2(saved_movement_1[2]) is not None:
        ...
        try:
            ...
            if path.startswith('/arnobot.jpeg'):
                ...
                if num == 1:
                    ...
                    if solve_d3(angle) is not None:
                        move_stepper(angle, joint2)
                    else:
                        print("Invalid d3")
                elif num == 3:
                        if solve_d2(angle) is not None:
                ...
                if saved_movement_2:
                    ...
                    if solve_d3(saved_movement_2[1]) is not None:
                        move_stepper(saved_movement_2[1], joint2)

                    if solve_d2(saved_movement_2[2]) is not None:
                        move_stepper(saved_movement_2gi[2], joint3)
